...
export default function GrudgeList(){
    ...
    return (
        <div style={{margin: '20px'}}>
            <h3 test="grudge-list-title" style={{margin: '20px 0px'}}>{title}</h3>
            <div>
            <TextField 
            label="Add Grudge" 
            variant="filled" 
            ...
            ref-attr="grudge-input"
            />
            </div>
            <Button ref-attr="add-grudge-button" onClick={addGrudge}>
                Add Grudge
            </Button>
            <ul test="grudge-list" style={{color: 'white', listStyleType: 'none'}}>
                {
                    grudges.length > 0 && (
                        grudges.map((g) => {
                           return ( 
            ...
            {
                grudges.length > 0 && <Button test="clear-button" onClick={clearGrudges}>Clear</Button>
            }
        </div>
    )
}